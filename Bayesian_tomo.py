...
class BayInfTomo():
    ...
    """
    def __init__(self, n_count, povm, nqubits, samples=10000, burn_in_per=0.5, deviation=0.05, rho_guess=None, tol=0.01, verbose=True):
    ...
    def burn_in_phase(self):
        ...
        for i in range(self.burn_in):
            thetha_cand = btool.sample_thetha_cand(self.dim, thetha, self.deviation)
            joint_posterior = btool.calculate_joint_posterior_ratio(thetha, thetha_cand, self.dim, self.povm, self.n_count)
            proposal_ratio = btool.calculate_ratio_proposal(thetha, thetha_cand, self.dim, self.deviation)
            ...
            if self.verbose == True and (i+1) % (self.burn_in/5) == 0:
