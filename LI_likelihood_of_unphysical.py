...
for q in n_qubits:
    had_exp = df[0][q-1]
    shots = [3**x for x in np.arange(7,14)]
    ...
    for n in shots:
        for x in purity_list:
            ...
            for i in range(n_states):
                sim = DataSimNqubit(q, n, purity=x, explicit=False, had_exp=had_exp, expect_key=expect_key, total_shot=True)
...
fig, ax = plt.subplots(figsize=(16,10))
data = np.genfromtxt(r'C:\Users\caspe\OneDrive\Desktop\PUK - Morten\Code\Quantum-State-Tomography\LI_data\Fidelity_3D_4_high_shot.csv',delimiter=',')
x=data[:,0]
...
cf = ax.contourf(X ,
                  Y , Z, levels=levels,
                  cmap=cmap)
ax.set_title('LI fidelity - 4 Qubits')
ax.set_xlabel('Purity')
